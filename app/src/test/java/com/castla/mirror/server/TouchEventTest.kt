...
class TouchEventTest {
    ...
    fun `TouchEvent stores all fields correctly`() {
        val event = MirrorServer.TouchEvent("down", 0.5f, 0.75f, 2)
    ...
    fun `TouchEvent data class equality`() {
        val e1 = MirrorServer.TouchEvent("down", 0.5f, 0.5f, 0)
        val e2 = MirrorServer.TouchEvent("down", 0.5f, 0.5f, 0)
        assertEquals(e1, e2)
    }
    ...
    fun `TouchEvent data class inequality`() {
        val e1 = MirrorServer.TouchEvent("down", 0.5f, 0.5f, 0)
        val e2 = MirrorServer.TouchEvent("up", 0.5f, 0.5f, 0)
        assertNotEquals(e1, e2)
    }

    @Test
    fun `TouchEvent copy works`() {
        val original = MirrorServer.TouchEvent("down", 0.5f, 0.5f, 0)
    ...
    fun `boundary coordinates are valid`() {
        ...
        val topLeft = MirrorServer.TouchEvent("down", 0.0f, 0.0f, 0)
        val bottomRight = MirrorServer.TouchEvent("down", 1.0f, 1.0f, 0)
